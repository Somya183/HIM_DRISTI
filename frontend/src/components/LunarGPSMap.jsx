...
export default function LunarGPSMap({ analysisData, selectedTarget = 'shackleton' }) {
  ...
  return (
    <div className="glass-card" style={{ padding: '20px', borderRadius: '16px', position: 'relative' }}>
      ...
      {/* DETAILED AI ICE LOCATIONS & VOLUME ESTIMATION BREAKDOWN TABLE */}
      <div style={{ marginTop: '18px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
          <h4 style={{ fontSize: '15px', color: '#38bdf8', margin: 0, display: 'flex', alignItems: 'center', gap: '6px', fontWeight: '700' }}>
            <Droplets size={18} color="#00f3ff" /> Image Analysis: Detected Water Ice Locations & Volume Estimation
          </h4>
          <span style={{ fontSize: '11px', color: '#94a3b8' }}>
            Total Detected Hotspots: <strong style={{ color: '#00f3ff' }}>{iceDeposits.length}</strong>
          </span>
        </div>

        <div style={{ overflowX: 'auto', borderRadius: '10px', border: '1px solid rgba(0, 243, 255, 0.25)', background: 'rgba(7, 10, 18, 0.6)' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px', textAlign: 'left', color: '#e2e8f0' }}>
            <thead>
              <tr style={{ background: 'rgba(15, 23, 42, 0.9)', borderBottom: '1px solid rgba(0, 243, 255, 0.3)', color: '#00f3ff' }}>
                <th style={{ padding: '10px 12px' }}>Ice Location Name</th>
                <th style={{ padding: '10px 12px' }}>Lunar GPS Coordinates</th>
                <th style={{ padding: '10px 12px' }}>Peak Ice Prob (%)</th>
                <th style={{ padding: '10px 12px' }}>Est. Volume (m³)</th>
                <th style={{ padding: '10px 12px' }}>Est. Mass (Tonnes)</th>
                <th style={{ padding: '10px 12px' }}>Distance from Rover</th>
            ...
            <tbody>
              {iceDeposits.map((dep, idx) => {
                const isSelected = selectedDepositId === dep.id;
                const distKm = dep.distance_from_landing_site_km || (dep.distance_from_landing_site_m ? (dep.distance_from_landing_site_m / 1000).toFixed(2) : '20.59');
                const distM = dep.distance_from_landing_site_m ? dep.distance_from_landing_site_m.toLocaleString() : (distKm * 1000).toLocaleString();
                ...
                return (
                  <tr
                    ...
                    style={{
                      ...
                      background: isSelected ? 'rgba(0, 243, 255, 0.12)' : idx % 2 === 0 ? 'transparent' : 'rgba(255, 255, 255, 0.02)',
                      transition: 'background 0.2s'
                    }}
                  >
                    <td style={{ padding: '10px 12px', fontWeight: 'bold', color: '#f8fafc', display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <Target size={14} color={idx === 0 ? '#00f3ff' : '#38bdf8'} />
                      {dep.name}
                    </td>
                    <td style={{ padding: '10px 12px', fontFamily: 'monospace', color: '#38bdf8' }}>
                      {dep.lunar_coords.lat}°S, {dep.lunar_coords.lon}°E
                    </td>
                    <td style={{ padding: '10px 12px', fontWeight: 'bold', color: dep.peak_confidence_pct >= 70 ? '#10b981' : dep.peak_confidence_pct >= 40 ? '#00f3ff' : '#f59e0b' }}>
                      {dep.peak_confidence_pct}%
                    </td>
                    <td style={{ padding: '10px 12px', color: '#e0f2fe' }}>
                      📦 {dep.volume_m3 ? dep.volume_m3.toLocaleString() : '23,728'} m³
                    </td>
                    <td style={{ padding: '10px 12px', color: '#c084fc', fontWeight: '600' }}>
                      ⚖️ {dep.mass_tonnes ? dep.mass_tonnes.toLocaleString() : '37,966'} t
                    </td>
                    <td style={{ padding: '10px 12px', color: '#34d399', fontWeight: 'bold' }}>
                      📏 {distKm} km <span style={{ fontSize: '10px', color: '#94a3b8', fontWeight: 'normal' }}>({distM} m)</span>
                    </td>
